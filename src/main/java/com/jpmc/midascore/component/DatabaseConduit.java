...
public class DatabaseConduit {
    ...
    public void receiveTransaction(Transaction transaction) {
        ...
        if (sender != null && recipient != null && sender.getBalance() >= transaction.getAmount()) {
            ...
            // 2. Update balances (Sender loses only transaction amount, recipient gets transaction + incentive)
            sender.setBalance(sender.getBalance() - transaction.getAmount());
            ...
            // 3. Save transaction record with the incentive amount
            TransactionRecord transactionRecord = new TransactionRecord(sender, recipient, transaction.getAmount());
            transactionRecord.setIncentive(incentiveAmount); // Make sure you added this setter in TransactionRecord.java
            transactionRepository.save(transactionRecord);
        }
    }
}